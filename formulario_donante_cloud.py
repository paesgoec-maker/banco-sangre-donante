...
Guarda en Supabase → sincroniza al sistema local del hospital
"""
...
# ── Verificar si ya envió hoy por DNI ────────────────────────
def ya_envio_hoy(dni: str) -> bool:
    hoy = datetime.date.today().isoformat()
    res = supabase.table("donantes_cloud") \
        .select("id") \
        .eq("dni", dni) \
...
with st.form("formulario_donante", clear_on_submit=False):
    ...
    with c2:
        ...
        ocupacion_otros       = st.text_input("Especifique ocupación") if ocupacion_op == "Otros" else ""
        centro_trabajo_op     = st.selectbox("Centro de trabajo", ["", "Independiente", "Sector Público", "Sector Privado", "Otros"])
        centro_trabajo_otros  = st.text_input("Especifique centro de trabajo") if centro_trabajo_op == "Otros" else ""
    ...
    if resp["q1"] == "Sí":
        resp["q1_detalle"] = st.text_input("¿Hace cuánto fue su última donación?")
    ...
    if resp["q9"] == "Sí":
        resp["q9_detalle"] = st.text_input("¿Qué recibió y hace cuánto?")
    ...
    if resp["q13"] == "Sí":
        resp["q13_detalle"] = st.text_input("¿A qué zona viajó?")
...
if enviar:
    ...
    if errores:
        st.error("Por favor corrija los siguientes campos:\n" + "\n".join(errores))
        st.stop()

    # Verificar si ya envió hoy
    if ya_envio_hoy(dni.strip()):
        st.warning(f"⚠️ El DNI **{dni}** ya tiene un registro de donación para el día de hoy. "
                   "Si cree que es un error, consulte al personal del banco de sangre.")
        st.stop()
    ...
    try:
        ...
        st.error(f"Error al guardar. Por favor intente de nuevo.\nDetalle: {e}")
